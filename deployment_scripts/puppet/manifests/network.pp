...
# VIP
###############################
$ns_iptables_start_rules = "iptables -A INPUT -i hapr-b -s ${baremetal_network} -d ${baremetal_vip} -p tcp -m multiport --dports 6385,8080 -m state --state NEW -j ACCEPT; iptables -A INPUT -i hapr-b -s ${baremetal_network} -d ${baremetal_vip} -m state --state ESTABLISHED,RELATED -j ACCEPT; iptables -A INPUT -i hapr-b -j DROP"
$ns_iptables_stop_rules = "iptables -D INPUT -i hapr-b -s ${baremetal_network} -d ${baremetal_vip} -p tcp -m multiport --dports 6385,8080 -j ACCEPT; iptables -D INPUT -i hapr-b -s ${baremetal_network} -d ${baremetal_vip} -m state --state ESTABLISHED,RELATED -j ACCEPT; iptables -D INPUT -i hapr-b -j DROP"
$baremetal_vip_data = {
  namespace      => 'haproxy',
  nic            => $baremetal_int,
...
l23network::l2::bridge { $baremetal_int:
  provider => 'lnx'
} ->
...
# Order
###############################
Firewall<||> -> Cluster::Virtual_ip<||> -> L23network::L2::Patch<||> -> Neutron_plugin_ml2<||> -> Neutron_agent_ovs<||> -> Openstack::Network::Create_network<||>
